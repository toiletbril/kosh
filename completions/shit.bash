...
_shit_complete()
{
  ...
  case $previous_word in
    -M|--mood)
      _shit_compgen -W "$mood_values" -- "$current_word"
      return
      ;;
    -L|--init-moods)
      _shit_compgen -W "$mood_values" -- "$current_word"
...
-T -A -E -R"
  ...
  if [[ $current_word == --* ]]; then
    _shit_compgen -W "$long_flags" -- "$current_word"
  elif [[ $current_word == -* ]]; then
    _shit_compgen -W "$short_flags $long_flags" -- "$current_word"
  else
    _shit_compgen -f -- "$current_word"
  fi
}
...
_shit_assimilate_complete()
{
  ...
  if [[ $previous_word == --link-mood ]]; then
    _shit_compgen -W "bash dash sh shit" -- "$current_word"
    return
  fi
  if [[ $current_word == -* ]]; then
    _shit_compgen -W "$flags" -- "$current_word"
    return
  fi
...
+a +b +e +f +h +k +m +n +u +v +x +B +C +E +P +T +A +R +W +WW +WWW +I +S +G"
  ...
  case $previous_word in
    -o|+o)
      _shit_compgen -W "$option_names" -- "$current_word"
      return
      ;;
    -M|--mood)
      _shit_compgen -W "$moods" -- "$current_word"
      return
      ;;
    -L|--init-moods)
      _shit_compgen -W "$moods" -- "$current_word"
  ...
  _shit_compgen -W "$switches" -- "$current_word"
}
...
_shitbox_complete()
{
  ...
  if [[ $COMP_CWORD -eq 1 ]]; then
    _shit_compgen -W "$_shitbox_utils --list --assimilate --help" \
      -- "$current_word"
    return
  fi
  ...
  if [[ $current_word == -* ]]; then
    _shit_compgen -W "$(_shitbox_util_flags "$util")" -- "$current_word"
  else
    _shit_compgen -f -- "$current_word"
  fi
}

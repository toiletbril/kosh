# Shitbox utility guidance

## Reuse the utility framework

Search `src/shitbox`, `Shitbox.hpp`, and `Shitbox.cpp` before adding behavior.
...
types. `REGISTER_SHITBOX_UTIL_FLAGS` publishes the list for execution, help,
and completion. `parse_util_operands` handles flags, `--`, and operand
locations. `SHITBOX_SHOW_HELP_AND_RETURN` handles help.
...
Use `report_soft_shitbox_error` for recoverable utility diagnostics. Use
...
`Shitbox.hpp`. Keep the count derived from the last enum value. The source file
belongs directly below `src/shitbox`. The source Makefile discovers it
automatically.
...
The canonical native test is `test/shit/shitbox_<utility>.shit` unless the
...
A new utility updates the root `AGENTS.md`, `docs/shit.1`, and
`completions/shit.bash`. Add its name to the existing shitbox completion data.

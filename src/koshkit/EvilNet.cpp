    ...
    Maybe<evilnet_sort_key> sort_key) throws -> void
{
  ...
  for (usize index = 0;
       index < sizeof(EVILNET_SORT_SPECS) / sizeof(EVILNET_SORT_SPECS[0]);
       index++)
  {
                                   ...
                                   evilnet_color_mode color_mode) throws -> usize
{
  ...
  append_titled_report_table(output, "Network interfaces", table, should_color);
    ...
    evilnet_color_mode color_mode) throws -> usize
{
  ...
  const StringView HEADERS[] = {
      receive_header.view(),
      transmit_header.view(),
      receive_packets_header.view(),
      transmit_packets_header.view(),
      receive_errors_header.view(),
      transmit_errors_header.view(),
      receive_drops_header.view(),
      transmit_drops_header.view(),
      "RX CAP",
      "TX CAP",
      "TX QUEUE",
      "TX LIMIT",
  ...
  for (let const &entry : statistics) {
    let const is_default =
        default_interface.has_value() &&
        entry.interface_name.view() == default_interface->view();
    ...
    -> bool
{
  ...
  constexpr StringView FAILURE_NAMES[] = {
      ...
      "sent resets",
      "connection drops",
      "receive memory drops",
      "listen drops",
      "retransmit timeouts",
      "listen overflows",
      "SYN retransmits",
      "fast retransmits",
      "spurious retransmit timeouts",
      "receive queue drops",
      "backlog drops",
      "request queue full drops",
      "retransmit failures",
  };
  const u64 failure_values[] = {
      ...
      statistics.listen_overflow_count,
      statistics.syn_retransmit_count,
      statistics.fast_retransmit_count,
      statistics.spurious_retransmit_timeout_count,
      statistics.receive_queue_drop_count,
      statistics.backlog_drop_count,
      statistics.request_queue_full_drop_count,
      statistics.retransmit_failure_count,
  };
  constexpr os::tcp_statistics_field FAILURE_FIELDS[] = {
      ...
      os::tcp_statistics_field::ListenOverflows,
      os::tcp_statistics_field::SynRetransmits,
      os::tcp_statistics_field::FastRetransmits,
      os::tcp_statistics_field::SpuriousRetransmitTimeouts,
      os::tcp_statistics_field::ReceiveQueueDrops,
      os::tcp_statistics_field::BacklogDrops,
      os::tcp_statistics_field::RequestQueueFullDrops,
      os::tcp_statistics_field::RetransmitFailures,
  };
...
fn sample_network_statistics(
    ...
    -> ArrayList<os::network_interface_statistics_entry>
{
    ...
    -> os::network_interface_statistics_entry
{
  ...
  do_sample(os::network_statistics_field::TransmitBytes,
            &os::network_interface_statistics_entry::transmit_bytes);
  do_sample(os::network_statistics_field::ReceivePackets,
            &os::network_interface_statistics_entry::receive_packet_count);
  do_sample(os::network_statistics_field::TransmitPackets,
            &os::network_interface_statistics_entry::transmit_packet_count);
  do_sample(os::network_statistics_field::ReceiveErrors,
            &os::network_interface_statistics_entry::receive_error_count);
  do_sample(os::network_statistics_field::TransmitErrors,
            &os::network_interface_statistics_entry::transmit_error_count);
  do_sample(os::network_statistics_field::ReceiveDrops,
            &os::network_interface_statistics_entry::receive_drop_count);
  do_sample(os::network_statistics_field::TransmitDrops,
            &os::network_interface_statistics_entry::transmit_drop_count);
  return sampled;
}

    ...
    -> i32
{
  ...
  if (!operands.is_empty() && (operands[0].is_empty() || operands[0][0] != '+'))
  {
    report_soft_koshkit_error(ec, cxt,
                              "date: setting the system clock is unsupported");
    return 1;
  }
  ...
  if (broken_down == nullptr)
    throw Error{"date: cannot convert the current time"};
  ...
  if (length == 0 && !format.is_empty())
    throw Error{"date: formatted output exceeds the supported line length"};

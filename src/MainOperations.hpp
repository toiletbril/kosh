...
constexpr u8 HISTORY_CONTROL_IGNORE_SPACE = 1U << 0;
constexpr u8 HISTORY_CONTROL_IGNORE_DUPLICATES = 1U << 1;
constexpr u8 HISTORY_CONTROL_ERASE_DUPLICATES = 1U << 2;

constexpr static_string_entry<u8> HISTORY_CONTROL_ENTRIES[] = {
    {SSK("ignorespace"), HISTORY_CONTROL_IGNORE_SPACE                },
    {SSK("ignoredups"),  HISTORY_CONTROL_IGNORE_DUPLICATES           },
    {SSK("ignoreboth"),
     HISTORY_CONTROL_IGNORE_SPACE | HISTORY_CONTROL_IGNORE_DUPLICATES},
    {SSK("erasedups"),   HISTORY_CONTROL_ERASE_DUPLICATES            },
};
constexpr StaticStringMap HISTORY_CONTROLS{HISTORY_CONTROL_ENTRIES};

static pure fn parse_history_control(StringView value) wontthrow -> u8
{
  u8 result = 0;
  usize start_position = 0;
  loop
  {
    usize end_position = start_position;
    while (end_position < value.length && value[end_position] != ':')
      end_position++;
    let const control = HISTORY_CONTROLS.find(value.substring_of_length(
        start_position, end_position - start_position));
    if (control.has_value()) result |= *control;
    if (end_position == value.length) break;
...
constexpr u8 HISTORY_IGNORE_COMMAND = 1U << 0;
constexpr u8 HISTORY_IGNORE_CONSECUTIVE_DUPLICATE = 1U << 1;

static fn match_history_ignore(StringView value, StringView command,
                               EvalContext &context) throws -> u8
{
  let const scratch_mark = context.scratch_mark();
  defer { context.scratch_release(scratch_mark); };
  let pattern = String{context.scratch_allocator()};
  let active = Bitset{context.scratch_allocator()};
  pattern.reserve(value.length);
  active.reserve(value.length);
  u8 result = 0;
  usize position = 0;
  ...
  loop
  {
    ...
    while (position < value.length && value[position] != ':') {
      if (value[position] == '\\' && position + 1 < value.length) {
        pattern.push(value[position + 1]);
        active.push(false);
        position += 2;
      } else {
        pattern.push(value[position++]);
        active.push(true);
      }
    }
    if (position < value.length) position++;

    if (pattern.count() == 1 && pattern[0] == '&' && active[0]) {
      result |= HISTORY_IGNORE_CONSECUTIVE_DUPLICATE;
    } else if (!pattern.is_empty() &&
               utils::glob_matches(pattern.view(), command, active, 0,
                                   context.extglob_enabled()))
    {
      return result | HISTORY_IGNORE_COMMAND;
    }
...
static fn interactive_history_duplicate_policy(StringView command,
                                               EvalContext &context) throws
    -> Maybe<toiletline::history_duplicate_policy>
{
  u8 control = 0;
  if (let const *value = context.lookup_shell_variable("HISTCONTROL");
      value != nullptr)
  {
    control = parse_history_control(value->view());
  }
  if ((control & HISTORY_CONTROL_IGNORE_SPACE) != 0 && !command.is_empty() &&
      command[0] == ' ')
  {
    return None;
  }
  ...
  u8 ignore = 0;
  if (let const *value = context.lookup_shell_variable("HISTIGNORE");
      value != nullptr && !value->is_empty())
  {
    let first_line = command;
    if (let const newline = first_line.find_character('\n');
        newline.has_value())
      first_line = first_line.substring_of_length(0, *newline);
    ignore = match_history_ignore(value->view(), first_line, context);
    if ((ignore & HISTORY_IGNORE_COMMAND) != 0) return None;
  }

  let const should_ignore_consecutive =
      (control & HISTORY_CONTROL_IGNORE_DUPLICATES) != 0 ||
      (ignore & HISTORY_IGNORE_CONSECUTIVE_DUPLICATE) != 0;
  let const should_erase_previous =
      (control & HISTORY_CONTROL_ERASE_DUPLICATES) != 0;
  if (should_ignore_consecutive && should_erase_previous)
    return toiletline::history_duplicate_policy::
        IgnoreConsecutiveAndErasePrevious;
  if (should_erase_previous)
    return toiletline::history_duplicate_policy::ErasePrevious;
  if (should_ignore_consecutive)
    return toiletline::history_duplicate_policy::IgnoreConsecutive;
